...
→ Читайте слово на кнопке: СРЕДНЯЯ СЛЕВА
...
- Для каждого провода указываете: цвет, светодиод, звездочку
- Программа использует диаграмму Венна и выдает решение для каждого провода
...
ПРОВОД 1:
  Цвет(а) провода: красный синий
  Горит ли светодиод? да
  Нарисована ли звездочка? нет
  → РЕЖЬТЕ

ПРОВОД 2:
  Цвет(а) провода: белый
  Горит ли светодиод? нет
  Нарисована ли звездочка? да
  → РЕЖЬТЕ
```
...
**Особенности:**
- Белый цвет игнорируется в логике диаграммы
- Бело-красный = только красный
- Бело-синий = только синий
- Красно-синий = оба цвета учитываются
...
**Как работает:**
- Вводите состояние 12 светодиодов (2 ряда по 6)
- Программа показывает ваш паттерн и все 4 эталонных паттерна
- Сравниваете визуально и определяете позицию ручки
...
где:
1-6 - номер колонки
в   - верхняя лампочка горит
н   - нижняя лампочка горит
вн  - обе горят
пусто - не горят
```
...
**Пример использования:**
```
Введите состояние: 1в 2вн 3н 4вн 5в 6
...
ПАТТЕРНЫ ИЗ МАНУАЛА:
[показываются все 4 паттерна]

→ Сравните и установите ручку в нужную позицию
```
...
5. **Simon Says** - используйте команду `показать` если забыли последовательность
6. **Память** - доверяйте программе, она запоминает все этапы
